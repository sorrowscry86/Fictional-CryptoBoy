...
class RedisStressTest:
    ...
    def test_parallel_updates(self, iterations: int = 10000, workers: int = 20, pairs: List[str] = None):
        ...
        logger.info(
            f"Starting parallel updates test: {iterations} operations, "
            f"{workers} workers, {len(pairs)} pairs"
        )
        self.results['start_time'] = time.time()
        ...
        report = {
            'summary': {
                ...
                'success_rate': round(
                    ...
                    (self.results['total_operations'] + self.results['failed_operations']) * 100,
                    2
                ) if (self.results['total_operations'] + self.results['failed_operations']) > 0 else 0
            },
            'write_latency_ms': {
                ...
                'p95': round(
                    statistics.quantiles(write_latencies, n=20)[18], 2
                ) if len(write_latencies) > 20 else 0,
                'p99': round(
                    statistics.quantiles(write_latencies, n=100)[98], 2
                ) if len(write_latencies) > 100 else 0,
            ...
            'read_latency_ms': {
                ...
                'p95': round(
                    statistics.quantiles(read_latencies, n=20)[18], 2
                ) if len(read_latencies) > 20 else 0,
                'p99': round(
                    statistics.quantiles(read_latencies, n=100)[98], 2
                ) if len(read_latencies) > 100 else 0,
...
def main():
    ...
    parser.add_argument('--mode', choices=['rapid', 'parallel', 'mixed'], default='parallel',
                        help='Test mode')
    parser.add_argument('--read-ratio', type=float, default=0.7,
                        help='Read ratio for mixed mode (0.0-1.0)')
