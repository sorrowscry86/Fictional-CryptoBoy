...
class CoinbaseValidator:
    ...
    def test_fetch_ticker(self, pair: str) -> Tuple[bool, Dict]:
        ...
        try:
            ...
            result = {"pair": pair, "success": False, "error": f"Network error: {str(e)}", "error_type": "NetworkError"}
    ...
    def test_fetch_ohlcv(self, pair: str, timeframe: str = "1h", limit: int = 10) -> Tuple[bool, Dict]:
        ...
        try:
            ...
            if not ohlcv or len(ohlcv) == 0:
                result = {"pair": pair, "success": False, "error": "No OHLCV data returned", "candles_received": 0}
            ...
            result = {"pair": pair, "success": False, "error": str(e), "candles_received": 0}
            self.print_error(f"{pair}: {result['error']}")
            return False, result
    ...
    def test_order_book(self, pair: str, limit: int = 10) -> Tuple[bool, Dict]:
        ...
        try:
            ...
            if result["best_bid"] and result["best_ask"]:
                result["spread"] = round(((result["best_ask"] - result["best_bid"]) / result["best_bid"]) * 100, 4)

                self.print_success(f"{pair}: Order book OK " f"(spread: {result['spread']}%, latency: {latency:.2f}ms)")
            else:
            ...
            result = {"pair": pair, "success": False, "error": str(e)}
            self.print_error(f"{pair}: {result['error']}")
            return False, result
    ...
    def run_test_1_fetch_live_market_data(self) -> bool:
        ...
        if ticker_success_rate >= 80:
            ...
            self.results["recommendations"].append("Market data fetch rate below 80% - check exchange connectivity")
            return False
    ...
    def run_test_2_verify_websocket(self) -> bool:
        ...
        try:
            ...
            self.results["tests"]["test_2_websocket"] = {"status": "ERROR", "error": "Docker command timeout"}
            return False
        except FileNotFoundError:
            self.print_warning("Docker not available in this environment")
            self.results["tests"]["test_2_websocket"] = {"status": "SKIP", "reason": "Docker not available"}
            return False
        except Exception as e:
            self.print_error(f"Error checking WebSocket: {e}")
            self.results["tests"]["test_2_websocket"] = {"status": "ERROR", "error": str(e)}
            return False
    ...
    def run_test_3_check_database(self) -> bool:
        ...
        try:
            ...
            self.results["tests"]["test_3_database"] = {"status": "SKIP", "reason": "Docker not available"}
            return False
        except Exception as e:
            self.print_warning(f"Database check skipped: {e}")
            self.results["tests"]["test_3_database"] = {"status": "SKIP", "reason": str(e)}
            return False
    ...
    def run_test_4_verify_services(self) -> bool:
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                self.results["tests"]["test_4_services"] = {"status": "ERROR", "error": result.stderr}
                return False
                ...
                if line:
                    try:
                        ...
                        if name in expected_services:
                            ...
                            service_status[name] = {"state": state, "status": status, "running": state == "running"}
            ...
            for service in expected_services:
                if service not in running_services:
                    ...
                    service_status[service] = {"state": "not_found", "running": False}
            ...
            self.results["tests"]["test_4_services"] = {"status": "SKIP", "reason": "Docker Compose not available"}
            return False
        except Exception as e:
            self.print_error(f"Service check failed: {e}")
            self.results["tests"]["test_4_services"] = {"status": "ERROR", "error": str(e)}
            return False
...
"""
            ...
            for ticker in test1.get("ticker_results", []):
                if ticker["success"]:
                    ...
                    report += (
                        f"| {ticker['pair']} | N/A | N/A | N/A | N/A | ✗ {ticker.get('error', 'Unknown error')} |\n"
                    )
