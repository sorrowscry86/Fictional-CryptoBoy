...
from data.market_data_collector import MarketDataCollector  # noqa: E402
...
class DataPipeline:
    ...
    def __init__(self):
        ...
        self.sentiment_analyzer = HuggingFaceFinancialSentiment(model_name="ProsusAI/finbert")
    ...
    def step1_collect_market_data(self, days: int = 365, timeframe: str = "1h"):
        ...
        for pair in self.trading_pairs:
            ...
            try:
                ...
                df = self.market_collector.update_data(symbol=pair, timeframe=timeframe, days=days)
    ...
    def step2_aggregate_news(self, max_age_days: int = 7):
        ...
        try:
            ...
            df = self.news_aggregator.update_news(filename="news_articles.csv", max_age_days=max_age_days)
    ...
    def step3_analyze_sentiment(self, news_df: pd.DataFrame):
        ...
        if not signals_df.empty:
            ...
            signals_df = (
                signals_df.drop_duplicates(subset=["pair", "article_id"])
            ...
            for pair in self.trading_pairs:
                ...
                if not pair_signals.empty:
                    ...
                    logger.info(
                        f"    {pair}: {len(pair_signals)} signals (↑{bullish} ↓{bearish} →{neutral}) avg={avg_score:.2f}"
                    )
        else:
    ...
    def run_full_pipeline(self, days: int = 365, max_news_age: int = 7):
        ...
        results = {"market_data": None, "news_data": None, "sentiment_signals": None, "success": False}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--step", type=str, choices=["1", "2", "3", "all"], default="all", help="Run specific step or all steps"
    )
