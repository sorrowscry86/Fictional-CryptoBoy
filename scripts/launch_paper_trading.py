...
def start_ollama():
    ...
    try:
        result = subprocess.run(
            ["docker-compose", "up", "-d", "ollama"], cwd=Path(__file__).parent.parent, capture_output=True, text=True
        )
...
def display_configuration():
    ...
    config = {
        ...
        "Sentiment Model": (
            os.getenv("HUGGINGFACE_MODEL", "finbert")
            if os.getenv("USE_HUGGINGFACE", "true") == "true"
            else os.getenv("OLLAMA_MODEL", "mistral:7b")
        ),
    }
